import {Component, ViewEncapsulation, Input, Inject, ElementRef, HostListener, Output, EventEmitter} from 'angular2/core';
import template from './activity-slider.html!text';
import styles from './activity-slider.css!text';

import CalendarTime from '../../pipes/calendar-time.js';
...
export default class ActivitySlider {
  // The input expects a list of acitivities
  @Input() activities;
  ...
  constructor(@Inject(ElementRef) elementRef) {
    ...
    // We'll need a reference to the overlay rectange so we can update its position and width
    this.selectionOverlay = this.sliderElement.shadowRoot.querySelector('.selection-overlay');
  ...
  // To prevent some browser issues with our selectio mode, we'll catch this event and prevent browser defaults
